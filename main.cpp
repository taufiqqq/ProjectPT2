...
public:
	Dish(string f, double p, string cat)
	{
		foodname = f;
		foodprice = p;
		category = cat;
...
public:
	...
	void update(Dish dishes[], int N)
	{
		...
		for (int i = 0; i < N; i++)
		{
			cout <<right<< setw(2) << i + 1  << ". " << left<<setw(8) << "Dish : " << setw(36) << dishes[i].getFoodName()  << 
        "| Price: "<< setw(5)<< dishes[i].getFoodPrice()  << 
        "| Category: " << dishes[i].getCategory() << endl;
		}
	}
};
...
public:
	Payment() {}
	Payment(double pc, string pId) : d() // Composition
	{
		price = pc;
		paymentId = pId;
	}
...
public:
	CreditCard(string card, string cvv, double price, string id) : Payment(price, id)
	{
		cardNum = card;
		cardCvv = cvv;
	} //
	void setCardNum(string n) { cardNum = n; }
	string getCardNum() const { return cardNum; }
	void setCardCvv(string m) { cardCvv = m; }
	string getCardCvv() const { return cardCvv; }
	double getPrice() override { return 1.1 * price; } // virtual
	void paymentMethod () override // virtual
	{
		cout << "+----------------------------------------------+" << endl;
		cout << "+----------------PAYMENT METHOD ---------------+" << endl;
		cout << "+----------------------------------------------+" << endl;
		cout << setfill(' ') << setw(20) << left << "Your payment method is Credit Card" << endl;
	}
};
...
public:
	Cash(string n , double price, string id) : Payment(price,id)
	 {
		IcNumber = n;
	 } //
	void setIcNumber(string n) { IcNumber = n; }
	string getIcNumber() const { return IcNumber; }
	double getPrice() override { return price; } // virtual
	void paymentMethod() override // virtual
	{
		cout << "+----------------------------------------------+" << endl;
		cout << "+----------------PAYMENT METHOD ---------------+" << endl;
		cout << "+----------------------------------------------+" << endl;
		cout << setfill(' ') << setw(20) << left << "Your payment method is Cash" << endl;
	}
};

class FPX : public Payment
{
private:
	string bankUsername;
	string bankPassword;
...
public:
	FPX(string username, string password, double price, string id) : Payment(price, id) 
  {
    bankUsername = username;
    bankPassword = password;
  } 
	void setbankUsername(string un) { bankUsername = un; }
	string getbankUsername() const { return bankUsername; }
	void setbankPassword(string p) { bankPassword = p; }
	string getbankPassword() const { return bankPassword; }
	double getPrice() override { return 1 + price; } // virtual
	void paymentMethod() override							 // virtual
	{
		cout << "+----------------------------------------------+" << endl;
		cout << "+----------------PAYMENT METHOD ---------------+" << endl;
		cout << "+----------------------------------------------+" << endl;
		cout << setfill(' ') << setw(20) << left << "Your payment method is FPX (Online Banking)" << endl;
	}
};
				...
				if (owner_name == admin.getusername() && owner_pass == admin.getpassword())
				{
					...
					case 1:
					{
						...
						while (menu_edit)
						{
							...
							{
								cout <<right<< setw(2) << i + 1  << ". " << left<<setw(8) << "Dish : " << setw(36) << dishes[i].getFoodName()  << 
        "| Price: "<< setw(5)<< dishes[i].getFoodPrice()  <<  "| Category: " << dishes[i].getCategory() << endl;
							}
						...
						do
						{
							...
							case 3:
							{
								if (tempPrice != 0)
								{
									...
									cout << "\nEnter number for the following option:" << endl;
									cout << "\n1. Credit Card (+10% charge for tipping)\n2. Cash\n3. FPX (+RM1 for service charge)\n4. Exit" << endl;
									cin >> paymentIndex;
									...
									if (paymentIndex == 1)
									{
										...
										do
										{
											...
											system("cls");
											if (cardnum.length() != 16 || cardcvv.length() != 3)
												cout << "Please enter a legitimate card and cvv number, thanks." << endl;

										} while (cardnum.length() != 16 || cardcvv.length() != 3);

										CreditCard c(cardnum, cardcvv, tempPrice, cus[customerIndex].getusername() + to_string(cus[customerIndex].pastOrder));
										a = &c;
										a->paymentMethod();
                    
										cout << endl;
										cout << "Your Card: " << c.getCardNum() << endl;
										cout << "Total Charge: " << a->getPrice() << endl; // will be extra 10% charge for polymorphism
										cout << "Payment ID: " << a->getPaymentId() << endl;
										cout << endl;
										cout << "+--------------------------------------------+" << endl;
									...
									else if (paymentIndex == 2)
									{
										...
										do
										{
											...
											cout << "+--------------------------------------------+" << endl;
											cout << left << "Please enter your IcNumber (eg:020416011415)=> " << endl;
											cin >> IcNumber;
											
											system("cls");
											if (IcNumber.length() != 12)
												cout << "Please enter a legitimate Ic number, thanks." << endl;
										} while (IcNumber.length() != 12);
										
                    Cash s(IcNumber, tempPrice, cus[customerIndex].getusername() + to_string(cus[customerIndex].pastOrder));
										a=&s;
										a->paymentMethod();

										cout << endl;
										cout << left << "  Ic Number : " << s.getIcNumber() << endl;
										cout << left << "  Total price : " << a->getPrice() << endl;//no charge for cash
										cout << left << "  Payment ID : " << a->getPaymentId() << endl;
										cout << endl;
										cout << "+--------------------------------------------+" << endl;
										cout << "+  THANK YOU, YOUR ORDER WILL BE DELIVERED   +" << endl;
										cout << "+    ACCORDING TO THE TIME/DATE PROVIDED     +" << endl;
									...
									else if (paymentIndex == 3)
									{
										...
										cout << "+--------------------------------------------+" << endl;
										cout << "+     Welcome to online banking section      +" << endl;
										cout << "+--------------------------------------------+" << endl;
										cout << "\nEnter your username: ";
										cin >> bankUsername;
										...
										system("cls");
                    FPX ob(bankUsername,bankPassword, tempPrice, cus[customerIndex].getusername() + to_string(cus[customerIndex].pastOrder));
										a=&ob;
										a->paymentMethod();

										cout << endl;
										cout << left << "  Name : " << cus[customerIndex].getusername()<< endl;
										cout << left << "  Total price : " << a->getPrice() << endl;
										cout << "Payment ID: " << a->getPaymentId() << endl;
										cout << endl;
										cout << "+--------------------------------------------+" << endl;
										cout << "+  THANK YOU, YOUR ORDER WILL BE DELIVERED   +" << endl;
									...
									d.requiredDelivery[customerIndex] = "Date: " + tempDate + " | Location: " + cus[customerIndex].getaddress() + " | Order:" + orderPtr->getcart(); 
									cus[customerIndex].pastOrder++;																	// track past order
									cus[customerIndex].history[cus[customerIndex].pastOrder - 1] = tempCart;						// Save order history
									tempPrice = 0;
									system("PAUSE");
								}
								else if (tempPrice <= 0)
